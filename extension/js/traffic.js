var apiKey = "Wevx7pnFUsE81pnfLU09D6M4QM042Ucx";
var centerCoords = [100.36379707549786, -0.9491813292632251];
var initialZoom = 11;
var maps = tt.map({
    key: apiKey,
    container: "map",
...
function removeBoundingBox() {
    if (maps.getSource(sourceID)) {
        maps.removeLayer(layerFillID);
        maps.removeLayer(layerOutlineID)
...
function onMouseDown(eventDetails) {
    if (drawBoundingBoxButtonPressed) {
        ...
        removeBoundingBox();
        maps.addSource(sourceID, getPolygonSource(startCornerLngLat, startCornerLngLat));
        maps.addLayer({
            id: layerFillID,
            type: "fill",
            source: sourceID,
...
function updateRectangleData(startCornerLngLat, endCornerLngLat) {
    maps.getSource(sourceID).setData(getPolygonSourceData(startCornerLngLat, endCornerLngLat));
}
...
function displayTrafficIncidents(boundingBox) {
    ...
    tt.services.incidentDetails({
            ...
            style: styleS1,
            zoomLevel: parseInt(maps.getZoom())
        })
        .go()
...
function initApplication() {
    ...
    maps.on("moveend", updateSearchBoxOptions);
}
